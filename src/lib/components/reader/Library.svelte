<script lang="ts">
	...
	async function handleClearLibrary() {
		...
		const message =
			`Are you sure you want to remove all ${library.length} ` +
			`book${library.length === 1 ? '' : 's'} from your library?\n\n` +
	...
	function handleFileInputChange(e: Event) {
		const file = validateFile(getFileFromInputEvent(e));
		if (file) {
	...
	<div class="sticky top-0 z-10 bg-[Canvas] px-6 py-4 shadow-sm">
		...
		<div class="mt-3 flex items-center gap-3">
			...
			{#if storageInfo.total > 0}
				<div
					class="h-1 flex-1 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"
				>
					<div
						class="h-full rounded-full transition-all {getStorageColor(
			...
			{#if library.length === 0}
				...
				>
					...
					>
						...
						<p
							class="mt-4 px-4 text-center text-sm font-medium text-gray-600 dark:text-gray-400"
						>
